...
async def test_iteration_cap_finalization_retains_current_hook():
    context = ReducingContext()
    provider = NRoundToolProvider(n_tool_rounds=1)
    hooks = ScriptedHooks({"provider:request": injection("FINAL_FACT"), "tool:post": injection("LAST_TOOL_FACT")})
    ...
    assert any("LAST_TOOL_FACT" in str(m.content) for m in provider.requests[-1].messages)
    assert len(context.requirements[-1]) == 2
